...
def main():
    ...
    for episode in range(EPISODE):
        ...
        ecci_client.send_message(payload, ["client1", "client2", "client3"])
        payload = {"feature_encoder": feature_encoder, "relation_network": relation_network}
        ecci_client.send_message(payload, ["client"])
        feature_encoder_scheduler.step(episode)
        relation_network_scheduler.step(episode)
        data_msg_queue = ecci_client.get_sub_data_pnew_ayload_queue()
        payload = {"sample_features": sample_features, "batch_features": batch_features, "batch_labels": batch_labels}
        loss_list = []
        for i in range(3):
        ...
        loss=(loss_list[0]+loss_list[1] +loss_list[2])/3
